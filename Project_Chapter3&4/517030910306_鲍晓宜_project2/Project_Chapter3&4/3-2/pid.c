...
static ssize_t proc_read(struct file *file, char __user *usr_buf, size_t count, loff_t *pos)
{
	...
	if (tsk==NULL){
		printk(KERN_INFO "pid = [%ld], it is not a valid process id. ", l_pid);
		rv = sprintf(buffer, "pid = [%ld], it is not a valid process id. \n", l_pid);
		completed = 1;
	}
	else{
		printk(KERN_INFO "command = [%s] pid = [%ld] state = [%ld]", tsk-> comm, l_pid, tsk -> state);
...
static ssize_t proc_write(struct file *file, const char __user *usr_buf, size_t count, loff_t *pos)
{
	...
	k_mem[j+1] = '\0';
	// printk(KERN_INFO "@%s@\n", k_mem);
	rv = kstrtol(k_mem, 10, &l_pid);
	printk(KERN_INFO "l_pid=%ld\n", l_pid);
        kfree(k_mem);
